...
class CI_messaging
{
    public function __construct()
    {
        ...
        // ------------------------------------------------------------------------
        require_once APPPATH.'config/messaging.php';

        $this->ci->load->model('Messaging_model','message');
        $this->ci->load->helper('language');
        $this->ci->lang->load('messaging');
    }
    ...
    function get_message($msg_id, $user_id)
    {
        ...
        if ($message = $this->ci->message->get_message($msg_id, $user_id))
        {
     ...
     * @param   integer  $user_id      REQUIRED
     * @param   boolean  $full_thread  OPTIONAL - If true, user will also see message from thread posted BEFORE user became participant
    ...
    function get_full_thread($thread_id, $user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        if ($message = $this->ci->message->get_full_thread($thread_id, $user_id, $full_thread, $order_by))
        {
            return $this->_success($message);
        }
    ...
    function get_all_threads($user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
		...
		$message = $this->ci->message->get_all_threads($user_id, $full_thread, $order_by);
        if (is_array($message))
        {
            return $this->_success($message);
        }
     ...
     *                           - message are grouped in threads.
     *
     * @param   integer  $user_id      REQUIRED
     * @param   boolean  $full_thread  OPTIONAL - If true, user will also see message from thread posted BEFORE user became participant
     * @param   string   $order_by     OPTIONAL
        ...
        if (is_array($message))
        {
            ...
            foreach ($message as $msg)
            {
                if ( ! isset($threads[$msg['thread_id']]))
                {
                    ...
                    $threads[$msg['thread_id']]['message']  = array($msg);
                }
                else
                {
                    $threads[$msg['thread_id']]['message'][] = $msg;
                }
            }
    ...
    function update_message_status($msg_id, $user_id, $status_id )
    {
        ...
        if ($this->ci->message->update_message_status($msg_id, $user_id, $status_id))
        {
    ...
    function add_participant($thread_id, $user_id)
    {
        ...
        if ( ! $this->ci->message->valid_new_participant($thread_id, $user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_EXISTS);
        }

        if ( ! $this->ci->message->application_user($user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_NONSYSTEM);
        }

        if ($this->ci->message->add_participant($thread_id, $user_id ))
        {
    ...
    function remove_participant($thread_id, $user_id)
    {
        ...
        if ($this->ci->message->remove_participant($thread_id, $user_id))
        {
    ...
    function send_new_message($sender_id, $recipients, $subject = '', $body = '', $priority = PRIORITY_NORMAL)
    {
        ...
        if (empty($recipients))
        {
            return array(
                ...
                'msg'  => lang('messaging_'.MSG_ERR_INVALID_RECIPIENTS)
            );
        }

        if ($thread_id = $this->ci->message->send_new_message($sender_id, $recipients, $subject, $body, $priority))
        {
    ...
    function reply_to_message($msg_id, $sender_id, $subject = '', $body = '', $priority = PRIORITY_NORMAL)
    {
        ...
        if ($new_msg_id = $this->ci->message->reply_to_message($msg_id, $sender_id, $body, $priority))
        {
    ...
    function get_participant_list($thread_id, $sender_id = 0)
    {
        ...
        if ($participants = $this->ci->message-> get_participant_list($thread_id, $sender_id))
        {
    ...
    /**
     * get_msg_count() - returns integer with count of message for user, by status. defaults to new message
     *
     * @param   integer  $user_id    REQUIRED
    ...
    function get_msg_count($user_id, $status_id = MSG_STATUS_UNREAD)
    {
        ...
        if (is_numeric($message = $this->ci->message->get_msg_count($user_id, $status_id)))
        {
            return $this->_success($message);
        }
    ...
    private function _general_error()
    {
        return array(
            ...
            'msg'  => lang('messaging_'.MSG_ERR_GENERAL)
        );
    }
}
